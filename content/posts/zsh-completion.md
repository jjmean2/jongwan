...
2️⃣ 부분에서 정의하는 함수의 본문 내용이 completion 로직일 것이다. completion 로직에는 현재까지 입력된 값을 보고, 다음에 올 값의 후보를 결정하는 내용이 들어 있다. 이 로직을 이해하는 것은 이번 노트의 관심사가 아니다. 이건 다음에 한번 공부해 보기로 하고, 여기서는 이 completion 로직을 zsh가 어떻게 가져다 쓰는 건지를 알아보려고 한다.
...
이렇게 명령에 연결된 자동완성 함수를 알려주는 기능을 하는 게 `compdef` 이다. 다음과 같이 실행할 수 있다.
...
현재 명령에 어떤 함수가 자동완성 함수로 등록되어 있는지 보려면 `_comps`라는 Associative Array (Map, Dictionary와 같은 자료구조) 변수를 이용할 수 있다. 다음과 같이 `pnpm` 키를 살펴보면, `_pnpm`이 나오는 것을 볼 수 있다.
...
{{<notice>}}
1️⃣ 주석으로 자동완성 함수를 어떤 명령과 매핑시켜야 할지 파악한다.
{{</notice >}}
...
{{<notice>}}
자동완성 함수 정의 스크립트가 자동완성 동작으로 호출될 때, 4️⃣ 실제 자동완성 함수를 호출한다.
{{</notice >}}
